...
class Vertex:
    ...
    def proportion_calculator(self):
        ...
        for w in self.get_connections():
            ...
            flows = cost.values()
            r.append(flows)
            rr = list(itertools.chain.from_iterable(r)) #Put multiple lists in one big list   
            rrr = list(set(rr)) #Eliminate duplicate elements in the lis 
            prop = float(cost[self.get_id()]) / float(sum(rrr))
            print 'prop: %s' %prop
